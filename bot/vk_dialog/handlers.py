...
async def _stage_problem(api, uid: int, text: str) -> None:
    """Пользователь задал вопрос — сохраняем и предлагаем ответ за 29 ₽."""
    ...
    try:
        link = create_payment_link(uid, "q29")
        offer = (
            f"Я вижу ваш вопрос, душа моя {_emo()}\n\n"
            f"Ответ на один вопрос — 29 ₽.\n\n"
            f"После оплаты я сразу отвечу вам лично и подробно.\n\n"
            f"✨ Оплатить — 29 ₽:\n{link}"
        )
    except Exception as e:
        logger.error("VK q29 payment link error: %s", e)
        offer = (
            f"Я вижу ваш вопрос, душа моя {_emo()}\n\n"
            f"Ответ на один вопрос — 29 ₽. Свяжитесь с нами для оплаты."
        )

    kb = _make_keyboard("✅ Оплатить 29 ₽", "❓ Расскажите подробнее", one_time=True)
    await _typing_medium(api, uid)
    if kb:
        await _send_with_keyboard(api, uid, offer, kb)
    else:
        await _send(api, uid, offer)


# ─── Статичные парирования VK ─────────────────────────────────────────────────

_VK_DEFLECT_GENERIC = [
    "Душа моя, я уже смотрю ваш вопрос — и чувствую, что за ним стоит что-то важное. Ссылка на оплату уже отправлена.",
    "Ваш вопрос глубже чем кажется. Именно поэтому я отвечаю вдумчиво — после оплаты 29 ₽ вы получите полный ответ.",
    "Я жду вас, душа моя. Ссылка на оплату открыта — 29 ₽ и я сразу начну отвечать.",
    "За каждым вопросом стоит своя история. Я готова разобрать вашу — ответ всего за 29 ₽.",
    "Я вижу в вашем вопросе несколько важных линий. Оплатите — и я разберу их для вас подробно.",
    "Ответы приходят тем, кто ищет. Я здесь — ссылка на оплату выше.",
]
_VK_DEFLECT_NO_MONEY = [
    "Понимаю вас, душа моя. Когда будете готовы — я здесь. Ссылка никуда не денется.",
    "29 ₽ — это меньше стакана воды, а ответ останется с вами надолго.",
...
_VK_DEFLECT_ONE_MORE = [
    "Маленьких вопросов не бывает — за каждым стоит целая жизнь. Оплатите 29 ₽ — и я отвечу.",
    "Один вопрос только открывает дверь. Я готова ответить — ссылка на оплату выше.",
    "За «ещё одним вопросом» всегда стоит «ещё один» — это и есть жизнь. Каждый за 29 ₽, я здесь.",
    "Душа моя, оплатите этот вопрос — и я отвечу полно и честно. Ссылка выше.",
]
_VK_DEFLECT_LATER = [
