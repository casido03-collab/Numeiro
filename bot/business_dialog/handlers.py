...
_DEFLECT_GENERIC = [
    ...
    "Ваш вопрос глубже чем кажется. Именно для такой работы я предлагаю вам быть рядом на целый месяц.",
    ...
    "Вы в правильном месте, душа моя. Чтобы я была рядом каждый день — оформите доступ.",
...
_DEFLECT_NO_MONEY = [
    ...
    "990 ₽ — это меньше одного похода в кафе, но целый месяц работы со мной каждый день. Когда будет возможность — я здесь.",
    "Понимаю. Жизнь бывает разной. Когда почувствуете готовность — просто напишите.",
...
_DEFLECT_ONE_MORE = [
    ...
    "Ещё один вопрос — это ещё одна нить вашей судьбы. Именно для таких нитей и создана наша совместная работа.",
    "За «ещё одним вопросом» всегда стоит «ещё один» — это и есть жизнь. Я рядом весь месяц.",
    "Всего один? Душа моя, за этим ответом придёт следующий вопрос. Именно для этого я предлагаю работу на месяц.",
]
_DEFLECT_LATER = [
    ...
    "Лучшего момента не будет, голубчик. Тот, кто ищет ответы — находит их. Ссылка открыта.",
    "Подождать — это выбор. Но ваш вопрос уже внутри, он не уйдёт. Я готова разобрать его сейчас.",
    "«Потом» — это слово которое откладывает ответы на важные вопросы. Я здесь прямо сейчас.",
]
_DEFLECT_DISCOUNT = [
    "Моя работа — это время и внимание, которые я отдаю каждому. Ценность не меняется.",
    "990 ₽ в месяц — это меньше чашки кофе в неделю. Зато каждый день я рядом с вами.",
    "Скидки нет, душа моя — потому что я вкладываю в каждый ответ всё своё внимание. Это честно.",
    "Я не провожу акции — я провожу настоящую работу. Ценность в этом.",
]
_DEFLECT_DOUBT = [
    ...
    "Сомнение — это честно. Но вы уже видели что я умею. Продолжим?",
]
_DEFLECT_SERVICE = [
    ...
    "В рамках подписки — столько вопросов сколько нужно, каждый день. Я всегда здесь.",
]

_NO_MONEY_KEYWORDS = ["нет денег", "нету денег", "денег нет", "не могу", "дорого", "финансы", "зарплата", "копейки", "накоплений", "нищ", "бедн", "пенсия", "студент"]
_ONE_MORE_KEYWORDS = ["ещё один", "еще один", "один вопрос", "маленький вопрос", "последний вопрос", "ещё раз", "ещё вопрос", "можно спросить", "можно задать", "разочек", "один разок", "только спросить", "один маленький", "уточнить"]
_LATER_KEYWORDS = ["потом", "позже", "подумаю", "подожди", "не сейчас", "завтра", "когда нибудь", "когда-нибудь", "после", "посоветуюсь", "посмотрю", "может быть", "возможно оплачу", "подумать", "время нужно"]
_DISCOUNT_KEYWORDS = ["скидк", "дешевле", "снизьте", "меньше", "за полцены", "акци", "промокод", "бесплатн"]
_DOUBT_KEYWORDS = ["не верю", "не верит", "откуда знаете", "придумано", "не угадали", "неправда", "не работает", "не точно", "сомневаюсь", "докажите"]
_SERVICE_KEYWORDS = ["сколько вопросов", "как быстро", "что входит", "как отменить", "как работает", "про что можно", "любой вопрос"]
...
def _detect_deflect_type(text: str) -> str:
    ...
    if any(k in t for k in _DISCOUNT_KEYWORDS):
        return "discount"
    if any(k in t for k in _DOUBT_KEYWORDS):
        return "doubt"
    if any(k in t for k in _SERVICE_KEYWORDS):
        return "service"
    return "generic"
...
async def _deflect_tg(bot: Bot, chat_id: int, telegram_id: int, biz_conn_id: str | None, text: str, dtype: str | None = None) -> None:
    ...
    _map = {
        "no_money": _DEFLECT_NO_MONEY,
        "one_more": _DEFLECT_ONE_MORE,
        "later":    _DEFLECT_LATER,
        "discount": _DEFLECT_DISCOUNT,
        "doubt":    _DEFLECT_DOUBT,
        "service":  _DEFLECT_SERVICE,
        "generic":  _DEFLECT_GENERIC,
    }
    reply = random.choice(_map.get(dtype, _DEFLECT_GENERIC))
...
_DAILY_LIMIT_RESPONSES = [
    ...
    "Прошу прощения, голубчик — сейчас я очень занята. Как только появится время — я вернусь и дам знать ✨",
    ...
    "Прошу немного потерпеть, голубчик — дел накопилось. Как только освобожусь — я здесь 💫",
    ...
    "Сейчас непростое время, голубчик — много дел. Я помню о вас и обязательно вернусь 🌟",
    "Душа моя, у меня сейчас накопилось немало забот. Как только разберусь — сразу к вам 🌙",
    "Прошу немного подождать — дел сейчас много, голубчик. Я обязательно вернусь ✨",
    ...
    "Сейчас у меня много дел, голубчик. Как только смогу — обязательно к вам ✨",
    "Простите, душа моя — накопилось много всего. Я не забываю о вас — вернусь обязательно 🌙",
    "Прошу немного подождать, голубчик — у меня сейчас много важного. Как только освобожусь — напишу 💫",
...
async def _stage_paid_monthly_tg(bot: Bot, chat_id: int, telegram_id: int, biz_conn_id: str | None, text: str) -> None:
    """Стадия для платных: 3 AI-вопроса в день, потом одно сообщение и молчим."""
    from bot.services.cache import get_redis
    ...
    if count >= _BIZ_DAILY_LIMIT:
        # Отправляем сообщение только ОДИН раз — потом молчим
        limit_notified_key = f"biz:limit_notified:{telegram_id}:{today}"
        already_notified   = await r.get(limit_notified_key)
        if not already_notified:
            # Выбираем сообщение по дню месяца чтобы каждый день новое
            day_of_month = datetime.now(timezone.utc).day
            msg = _DAILY_LIMIT_RESPONSES[(day_of_month - 1) % len(_DAILY_LIMIT_RESPONSES)]
            await typing_short(bot, chat_id, biz_conn_id)
            await _send(bot, chat_id, msg, biz_conn_id)
