...
async def _stage_waiting_upsell(
    ...
    if next_tier_key == "t990":
        reply = await generate_business(
            ...
            f"Сейчас предложен следующий шаг — расклад карт Таро, который даёт "
            f"прямой конкретный ответ на вопрос клиента.\n\n"
            ...
            f"шага — это был фундамент (причины, энергии). Карты Таро дадут ей ПРЯМОЙ "
            f"конкретный ответ именно на её вопрос — не общие слова, а образ ситуации.\n\n"
            f"• СОМНЕНИЕ / ВОПРОС «зачем»: «чем отличается», «стоит ли», «а что даст».\n"
            f"→ Ответ: объясни разницу между разбором причин и Таро-прогнозом.\n\n"
